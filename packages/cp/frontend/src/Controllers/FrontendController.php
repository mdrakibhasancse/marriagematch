...
class FrontendController extends Controller
{
    ...
    {
        $data['posts'] = BlogPost::whereActive(true)->whereStatus('published')->latest()->paginate(12);
        ...
        // $data['recent_posts'] = BlogPost::latest()->whereActive(true)->whereStatus('published')->latest()->take(6)->get();
    ...
    public function blogCategory($slug)
    {
        ...
        return view('frontend::welcome.blogCategory', compact('category', 'posts'));
    }

    public function blogTag($tag)
    {
        $posts = BlogPost::where('tags', 'like', '%' . $tag . '%')->paginate(16);
        return view('frontend::welcome.blogTag', compact('tag', 'posts'));
    }
    ...
    public function singlePost($slug)
    {
        ...
        $data['tags'] = $post->tags ? explode(", ", $post->tags) : null;

        $postCategories = $post->blogCategories->pluck('id');
        $postIds = DB::table('blog_category_posts')->whereIn('blog_category_id', $postCategories)->pluck('blog_post_id');

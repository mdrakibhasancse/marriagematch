...
//     return view('user.linkpreview.postlinkpreview', ['linkPrevs' => $preview , 'link' => $text])->render();
// }
...
function professions($gender)
{
  $cProf = config('parameter.profession_common');
  if ($gender == 'female') {
    $fProf = config('parameter.profession_female');
  } else {
    $fProf = config('parameter.profession_male');
  }
  $array = Arr::collapse([$fProf, $cProf]);
